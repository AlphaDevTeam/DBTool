...
type SelectableEntity = IUser | ICompany | IBranch | IUserGroup | IUserPermissions | IExUserAuth;

type SelectableManyToManyEntity = IBranch | IUserGroup | IUserPermissions | IExUserAuth;
...
export class ExUserUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ exUser }) => {
      ...
      this.exUserAuthService
        .query()
        .pipe(
          map((res: HttpResponse<IExUserAuth[]>) => {
            return res.body ? res.body : [];
          })
        )
        .subscribe((resBody: IExUserAuth[]) => (this.exuserauths = resBody));
    });
  }
  ...
  private createFromForm(): IExUser {
    return {
      ...
      userAuths: this.editForm.get(['userAuths'])!.value
    };
  }
