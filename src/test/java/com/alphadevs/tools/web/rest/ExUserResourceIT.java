...
import com.alphadevs.tools.domain.User;
...
public class ExUserResourceIT {
    ...
    @Test
    @Transactional
    public void getAllExUsersByBranchIsEqualToSomething() throws Exception {
        // Get already existing entity
        Branch branch = exUser.getBranch();
        exUserRepository.saveAndFlush(exUser);
        Long branchId = branch.getId();

        // Get all the exUserList where branch equals to branchId
        defaultExUserShouldBeFound("branchId.equals=" + branchId);

        // Get all the exUserList where branch equals to branchId + 1
        defaultExUserShouldNotBeFound("branchId.equals=" + (branchId + 1));
    }
    ...
    @Test
    @Transactional
    public void getAllExUsersByUserAuthIsEqualToSomething() throws Exception {
        // Initialize the database
        exUserRepository.saveAndFlush(exUser);
        ExUserAuth userAuth = ExUserAuthResourceIT.createEntity(em);
        em.persist(userAuth);
        em.flush();
        exUser.addUserAuth(userAuth);
        exUserRepository.saveAndFlush(exUser);
        Long userAuthId = userAuth.getId();

        // Get all the exUserList where userAuth equals to userAuthId
        defaultExUserShouldBeFound("userAuthId.equals=" + userAuthId);

        // Get all the exUserList where userAuth equals to userAuthId + 1
        defaultExUserShouldNotBeFound("userAuthId.equals=" + (userAuthId + 1));
    }
