...
import com.alphadevs.tools.repository.CompanyRepository;
import com.alphadevs.tools.service.CompanyService;
import com.alphadevs.tools.web.rest.errors.ExceptionTranslator;
import com.alphadevs.tools.service.dto.CompanyCriteria;
import com.alphadevs.tools.service.CompanyQueryService;
...
public class CompanyResourceIT {
    ...
    @Test
    @Transactional
    public void getAllCompaniesByConfigitemsIsEqualToSomething() throws Exception {
        // Initialize the database
        companyRepository.saveAndFlush(company);
        ConfigurationItems configitems = ConfigurationItemsResourceIT.createEntity(em);
        em.persist(configitems);
        em.flush();
        company.addConfigitems(configitems);
        companyRepository.saveAndFlush(company);
        Long configitemsId = configitems.getId();

        // Get all the companyList where configitems equals to configitemsId
        defaultCompanyShouldBeFound("configitemsId.equals=" + configitemsId);

        // Get all the companyList where configitems equals to configitemsId + 1
        defaultCompanyShouldNotBeFound("configitemsId.equals=" + (configitemsId + 1));
    }
